...
takeSteps :: Set Point -> [[Point]] -> Int -> Int
takeSteps elves steps moves
  | elves == elves' = moves
  | otherwise = takeSteps elves' (drop 1 steps) (moves + 1)
...
solve = do
  ...
  let elves = takeSteps points steps 1

  printf "Stops at: %d" elves
