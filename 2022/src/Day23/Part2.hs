...
module Day23.Part2 (solve) where
    ...
    getDestinations A = [(3,2), (3,3), (3, 4), (3, 5)]
    getDestinations B = [(5,2), (5,3), (5, 4), (5, 5)]
    getDestinations C = [(7,2), (7,3), (7, 4), (7, 5)]
    getDestinations D = [(9,2), (9,3), (9, 4), (9, 5)]
    ...
    canMove grid (point, Entity (i, active))
      ...
      | not active = all ((\c -> isEmpty c || isSpecific i c) . (grid !)) (getDestinations i)
    ...
    dijkstra' queue neighbours done distMap prev
      ...
      | otherwise = dijkstra' queue'' neighbours done distMap' prev'
        where ((dist, cur), queue') = deleteFindMin queue
              next = case dist `mod` 100 of
                       0 -> trace (show dist) $ neighbours cur
                       _ -> neighbours cur
    ...
    solve = do
        d <- lines <$> getContents
        let d' = take 3 d ++ ["  #D#C#B#A#  ","  #D#B#A#C#  "] ++ drop 3 d
