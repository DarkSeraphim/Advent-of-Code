...
import Data.Functor (($>))
import Lib (newPoint, PointBig, getY, getX)
...
placePiece :: Set Point -> [Piece] -> [Dir] -> Integer -> Set Point
placePiece _ [] _ _ = error "Pieces should be infinite"
placePiece placements pieces gas 0 = placements
placePiece placements (piece:pieces') gas number = placePiece placements' pieces' gas' (number - 1)
