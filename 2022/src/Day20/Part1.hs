...
solve = do
  nums <- zip [0 :: Int ..] <$> parseInput parse
  ...
  let first = head nums
  let values = reconstructList first first nodes
  let nodes' = foldl (flip shuffle) nodes values
  let first = head $ filter (\t -> 0 == snd t) (keys nodes')
  let finalList = map snd $ reconstructList first first nodes'
  let a = map (\n -> cycle finalList !! max 0 n) [1000, 2000, 3000]
  printf "List: %s\n" (show $ sum a)
