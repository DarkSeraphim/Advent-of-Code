...
module Day19.Part2 (solve) where
    ...
    relativeAll' :: Map Beacon Beacon -> Map Beacon Point -> [Beacon] -> Map Beacon Point
    relativeAll' _ origins [] = origins
    relativeAll' prevs origins (next@Beacon {num, readings} : rest) = relativeAll' prevs origins' rest
    ...
    relativeAll :: Map Beacon Beacon -> [Beacon] -> Map Beacon Point
    ...
    solve = do
        ...
        let origins = elems good
        let biggest = maximum $ concatMap (\a -> map (distance a) origins) origins
